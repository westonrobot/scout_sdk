...
#include "agilex_protocol/parser/hunter_serial_parser.hpp"
...
namespace wescore
{
void HunterSerialParser::PackMotionControlMsgToBuffer(const MotionControlMessage &msg, uint8_t *buf, uint8_t &len)
{
    // SOF
    buf[0] = FRAME_SOF1;
    buf[1] = FRAME_SOF2;
...
uint8_t HunterSerialParser::CalcBufferedFrameChecksum()
{
...
void HunterSerialParser::PrintStatistics()
{
...
void HunterSerialParser::ParseChar(uint8_t c)
{
...
HunterStatusMessage HunterSerialParser::ConstructMessage()
{
    HunterStatusMessage msg;
    switch (frame_id_)
    {
    case FRAME_SYSTEM_STATUS_ID:
    {
        msg.updated_msg_type = HunterSystemStatusMsg;
    ...
    case FRAME_MOTION_STATUS_ID:
    {
        msg.updated_msg_type = HunterMotionStatusMsg;
...
void HunterSerialParser::ParseBuffer(uint8_t *buf, size_t len)
{
    for (int i = 0; i < len; ++i)
        ParseChar(buf[i]);
...
void HunterSerialParser::DefaultReceiveCallback(const HunterStatusMessage &msg)
{
